...
async function registerRoutes(app2) {
  ...
  app2.get("/api/property-details/:id/comments", async (req, res) => {
    try {
      const result = await pool.query(
  ...
  app2.post("/api/property-details/:id/comments", async (req, res) => {
    try {
      const userId = req.session?.userId;
      if (!userId) {
      ...
      const result = await pool.query(
        ...
        [parseInt(req.params.id), parent_id || null, userName, userEmail, userAvatar, comment.trim()]
      );
      res.json(result.rows[0]);
    } catch (e) {
      res.status(500).json({ error: e.message });
